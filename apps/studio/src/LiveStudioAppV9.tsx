...
import { createSourceBindingCommand } from "@afrodite/canvas-engine/binding";
import {
  cachePatchPlan,
  cacheSourceSnapshot,
  cacheWriteResult,
...
import { BindingManagerPanel } from "./BindingManagerPanel";
...
const initialDocument = parseUiDocument({
  schemaVersion: 1,
  id: "document.demo",
  name: "Afrodite visual binding session",
  root: {
    id: "node.workspace",
    kind: "element",
    element: "main",
...
export function LiveStudioAppV9() {
  ...
  createEffect(() => setJsonDraft(serializeUiDocument(document())));
  ...
  const updateLayout = (patch: LayoutPatch, label: string) => {
    ...
    const nextNode = findNode(command.apply(currentDocument).root, node.id);
    if (!nextNode) return;
  ...
  const confirmSourceBinding = (binding: SourceBinding) => {
    const node = selectedNode();
    if (!node) return;
    const command = createSourceBindingCommand(
      document(),
      node.id,
      binding,
      `Bind ${node.name} to ${binding.repositoryPath}`,
    );
    commit(command, command.label, { invalidateAllPatchState: true });
  };
  ...
  const planBlocked = createMemo(() => !patchPlan()?.changed
    || patchPlan()?.diagnostics.some((diagnostic) => diagnostic.severity === "error") === true);

  return (
    <div class="live-studio-root">
      <nav class="workspace-switcher" aria-label="Afrodite workspace">
        <button classList={{ active: session().workspace === "canvas" }} onClick={() => setWorkspace("canvas")}>Canvas</button>
        <button classList={{ active: session().workspace === "source-sync" }} onClick={() => setWorkspace("source-sync")}>Source Sync</button>
      </nav>
      ...
      <div class="session-telemetry" aria-label="Live project session status">
        ...
        <span>{boundNodes().length} bindings</span>
      </div>

      <Show when={session().workspace === "canvas"} fallback={
        <div class="source-sync-shell">
          <header class="source-sync-header">
            <div class="brand"><strong>Afrodite</strong><span>Verified source synchronization</span></div>
          ...
          <main class="source-sync-grid">
            <aside class="sync-panel sync-nodes">
              ...
              <p class="panel-hint">Bindings are created and reviewed in the Canvas Inspector.</p>
              <div class="sync-node-list">
                <For each={boundNodes()}>
                  {(node) => (
                    <button
                      classList={{ "sync-node": true, selected: node.id === selectedBoundNode()?.id }}
              ...
              <section class="sync-card transition-audit">
                <div class="section-heading"><h2>Session audit</h2><span>{session().layoutTransitions.length}</span></div>
                <For each={[...session().layoutTransitions].reverse().slice(0, 8)}>
                  {(transition) => (
                    <div class="transition-row">
                      <code>r{transition.revision}</code><span>{transition.phase}</span><strong>{transition.nodeId}</strong>
                    </div>
                  )}
                </For>
              </section>
            ...
            <section class="sync-workspace">
              <section class="sync-card bridge-card">
                <div class="section-heading"><h2>Local project bridge</h2><span>{bridgeHealth() ? `connected · ${bridgeHealth()!.projectName}` : "offline"}</span></div>
                <div class="bridge-fields">
                  ...
                  <button class="primary" disabled={bridgeBusy() || bridgeToken().length < 16} onClick={() => void connectBridge()}>Connect</button>
                </div>
              </section>

              <Show when={selectedBoundNode()} keyed fallback={<section class="sync-card sync-empty">Create or select a source binding in Canvas first.</section>}>
                {(node) => (
                  <>
                    <section class="sync-card operation-card">
                      <div class="section-heading"><h2>Recorded visual operation</h2><span>{node.sourceBinding.stableMarker ?? "marker missing"}</span></div>
                      ...
                      <Show when={pendingOperation()} keyed fallback={<p class="panel-hint">No unsynchronized layout transitions for this node.</p>}>
                        {(pending) => (
                          <div class="operation-comparison">
                            <div><strong>Before · r{pending.fromRevision}</strong><pre>{JSON.stringify(pending.before, null, 2)}</pre></div>
                            <div><strong>After · r{pending.toRevision}</strong><pre>{JSON.stringify(pending.after, null, 2)}</pre></div>
                          </div>
                        )}
                      </Show>
                      <div class="sync-actions">
                        <button disabled={bridgeBusy() || !bridgeHealth()} onClick={() => void readCurrentSource()}>Refresh source</button>
                        <button class="primary" disabled={bridgeBusy() || !bridgeHealth() || !pendingOperation()} onClick={() => void planVisualChanges()}>Plan visual diff</button>
                      </div>
                    </section>

                    <Show when={sourceSnapshot()} keyed>
                      {(source) => (
                        <details class="sync-card source-card">
                          <summary>Current source · {source.repositoryPath} · {source.version}</summary>
                          <pre class="source-view">{source.content}</pre>
                        </details>
                      )}
                    </Show>

                    <Show when={patchPlan()} keyed>
                      {(plan) => (
                        <section class="sync-card diff-card">
                          ...
                          <label class="approval-check">
                            <input
                              type="checkbox"
                              checked={approvedPlanId() === plan.planId}
                              disabled={planBlocked()}
                              onChange={(event) => setApprovedPlanId(event.currentTarget.checked ? plan.planId : "")}
                            />
                            I reviewed this exact plan ID and source version.
                          </label>
                          <button class="primary" disabled={bridgeBusy() || planBlocked() || approvedPlanId() !== plan.planId} onClick={() => void applyVisualChanges()}>Approve exact diff & apply</button>
                        </section>
                      )}
                    </Show>

                    <Show when={writeResult()} keyed>
                      {(result) => (
                        <section class={`sync-card result-card status-${result.status}`}>
                          <div class="section-heading"><h2>Write result</h2><span>{result.status}</span></div>
                          <For each={result.verification}>
        ...
        <div class="studio-shell">
          <header class="topbar">
            <div class="brand"><strong>Afrodite</strong><span>Visual Binding Manager</span></div>
          ...
          <div class="studio-grid vs003-grid">
            <aside class="panel library-panel">
              <section class="library-section">
                <div class="section-heading"><h2>Components</h2><span>{catalog().components.length}</span></div>
                ...
                <For each={nodes()}>
                  {({ node, depth }) => (
                    <button
                      ...
                      onClick={() => selectNode(node.id)}
                    >
                      <span class="layer-kind">{node.kind === "component" ? "C" : "E"}</span>{node.name}
                    </button>
                  )}
                </For>
              </section>
            ...
            <main class="canvas-panel composition-panel">
              <section class="workspace-pane">
                <div class="pane-heading"><div><strong>Semantic canvas</strong><span>Framework-neutral live UI IR</span></div><code>{document().id}</code></div>
                <div class="canvas-frame"><NodePreview node={document().root} selectedId={selectedId()} onSelect={selectNode} /></div>
              </section>
              <section class="workspace-pane runtime-pane">
                <div class="pane-heading"><div><strong>Runtime preview</strong><span>Trusted SolidJS + React runtimes</span></div><code>{previewReady() ? previewRequestId() : "connecting"}</code></div>
                <iframe
                ...
                <Show when={previewDiagnostics().length > 0}>
                  <div class="preview-result diagnostics"><For each={previewDiagnostics()}>{(diagnostic) => <p><code>{diagnostic.code}</code>{diagnostic.message}</p>}</For></div>
                </Show>
              </section>
            </main>
                ...
                {(node) => (
                  <>
                    <div class="node-heading">
                      <strong>{node.name}</strong><span class="node-id">{node.id}</span>
                      <Show when={node.sourceBinding}><code class="source-binding">{node.sourceBinding?.repositoryPath} · {node.sourceBinding?.frameworkId}</code></Show>
                    </div>

                    <section class="inspector-section">
                      <h3>Layout</h3>
                      <label>Display
                        <select value={node.layout.display} onChange={(event) => updateLayout({ display: event.currentTarget.value as Layout["display"] }, `Set display to ${event.currentTarget.value}`)}>
                          <option value="block">Block</option><option value="flex">Flex</option><option value="grid">Grid</option>
                        </select>
                      </label>
                      <label>Direction
                        <select value={node.layout.direction} disabled={node.layout.display !== "flex"} onChange={(event) => updateLayout({ direction: event.currentTarget.value as LayoutDirection }, `Set direction to ${event.currentTarget.value}`)}>
                          <option value="row">Row</option><option value="column">Column</option>
                        </select>
                      </label>
                      <div class="field-grid">
                        <NumberField label="Gap" value={node.layout.gap ?? 0} onChange={(value) => updateLayout({ gap: value }, `Set gap to ${value}px`)} />
                        <NumberField label="Padding" value={node.layout.padding ?? 0} onChange={(value) => updateLayout({ padding: value }, `Set padding to ${value}px`)} />
                      </div>
                      <SizingField axis="width" value={node.layout.sizing.width} onModeChange={changeSizingMode} onFixedChange={(axis, value) => updateLayout({ sizing: axis === "width" ? { width: value } : { height: value } }, `Set ${axis} to ${value}px`)} />
                      <SizingField axis="height" value={node.layout.sizing.height} onModeChange={changeSizingMode} onFixedChange={(axis, value) => updateLayout({ sizing: axis === "width" ? { width: value } : { height: value } }, `Set ${axis} to ${value}px`)} />
                    </section>
                    ...
                    <BindingManagerPanel
                      ...
                      onBindingConfirmed={confirmSourceBinding}
                      onStatus={setStatus}
                    />

                    <section class="inspector-section document-section">
                      <div class="section-heading"><h3>Document JSON</h3><span>revision {session().revision}</span></div>
                      <textarea value={jsonDraft()} spellcheck={false} onInput={(event) => setJsonDraft(event.currentTarget.value)} />
                      <Show when={diagnostics().length > 0}>
                        <div class="diagnostics"><For each={diagnostics()}>{(diagnostic) => <p><code>{diagnostic.path}</code>{diagnostic.message}</p>}</For></div>
                      </Show>
...
function NumberField(props: { label: string; value: number; onChange: (value: number) => void }) {
  return (
    <label>{props.label}<div class="number-input"><input type="number" min="0" step="1" value={props.value} onChange={(event) => props.onChange(toNonNegativeNumber(event.currentTarget.value))} /><span>px</span></div></label>
  );
}
...
function SizingField(props: {
  ...
  return (
    <div class="sizing-field">
      <label>{capitalize(props.axis)}
        <select value={mode()} onChange={(event) => props.onModeChange(props.axis, event.currentTarget.value as SizingMode)}>
      ...
      <Show when={typeof props.value === "number"}>
        <div class="number-input fixed-size-input"><input type="number" min="0" step="1" value={typeof props.value === "number" ? props.value : 0} onChange={(event) => props.onFixedChange(props.axis, toNonNegativeNumber(event.currentTarget.value))} /><span>px</span></div>
      </Show>
    </div>
  );
}
...
function NodePreview(props: { node: UiNode; selectedId: string; onSelect: (id: string) => void }) {
  ...
  return (
    <div
      classList={{ "preview-node": true, selected: props.node.id === props.selectedId, component: props.node.kind === "component" }}
      style={style()}
...
function restoreSavedDocument(): { document: UiDocument; status: string; diagnostics: readonly UiDiagnostic[] } {
  ...
  const decoded = decodeUiDocument(source);
  if (!decoded.ok) return { document: initialDocument, status: "Saved document was invalid; loaded sample", diagnostics: decoded.diagnostics };
...
function resultMessage(result: BridgeApplyResult): string {
  switch (result.status) {
    case "applied": return "Source patch applied and required verification passed";
    case "rejected": return "Source patch was rejected";
    case "rolled-back": return "Verification failed and the original source was restored";
    case "rollback-failed": return "Verification failed and rollback failed; inspect the source immediately";
  }
}
